...
export class Player {
  ...
  private readonly scale = 1.8; // CesiumMan is about 1m tall; scaling to 1.8 makes him human-sized
  private readonly boundaryLimit = 19; // Rooftop half-size is 20, clamp to 19 to keep player safe
  ...
  constructor(scene: THREE.Scene, emitter: EventEmitter) {
    ...
    emitter.on('input:keys', (keys: Record<string, boolean>) => {
      this.keys = keys;
    });
  }
