...
export class GameScene {
    ...
    constructor() {
        ...
        this.camera = new THREE.PerspectiveCamera(CAMERA.FOV_DEFAULT, window.innerWidth / window.innerHeight, CAMERA.NEAR, CAMERA.FAR);
        this.buildScene();
    }
    buildScene() {
        this.setupFog();
    ...
    setupFog() {
        this.scene.fog = new THREE.Fog(0x0a0a14, ENVIRONMENT.FOG_NEAR, ENVIRONMENT.FOG_FAR);
    ...
    buildRooftop() {
        ...
        const floor = new THREE.Mesh(new THREE.BoxGeometry(s, 0.3, s), this.materials.floor);
    ...
    addWindows(building, w, h, d, side) {
    ...
    buildStreetLights() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            this.scene.add(pole);
            const head = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 8), this.materials.head);
    ...
    setTheme(theme) {
        if (theme === 'light') {
            if (this.scene.fog) {
                this.scene.fog.color.setHex(0xcceeff);
            }
